...
|--------|-----|-----|--------|-----|
| Accept-cancellation endpoint | Implicit Update with RequestStatus=Accepted + outcome | -- | **Add `ApproveCancellationAsync(Guid changeRequestId, ApproveCancelDto { Outcome })`** | B |
| Reject-cancellation endpoint | Implicit Update with CancellationRejectionReason | -- | **Add `RejectCancellationAsync(Guid changeRequestId, RejectDto { Reason })`** | B |
| Accept-reschedule endpoint | Implicit Update | -- | **Add `ApproveRescheduleAsync(Guid changeRequestId, ApproveRescheduleDto { Outcome, OverrideSlotId?, AdminReason? })`** | B |
| Reject-reschedule endpoint | Implicit Update | -- | **Add `RejectRescheduleAsync(Guid changeRequestId, RejectDto { Reason })`** | B |
| Pending list endpoint with filters | OLD stored proc | -- | **Add `GetPendingChangeRequestsAsync(GetChangeRequestsInput)` with filters** | I |
| Outcome buckets are supervisor's call | OLD: free choice | -- | **Don't auto-derive from `AppointmentCancelTime`**; expose as DTO field | -- |
| `AdminReScheduleReason` required when supervisor overrides | OLD: bool check | -- | **Validate**: if `OverrideSlotId != original`, `AdminReason` is required | I |
| New Appointment row on reschedule-approve with cascade-copy | OLD complex flow | -- | **Implement `AppointmentManager.CloneForRescheduleAsync`** -- detailed in reschedule audit | B |
| Slot transitions: Reserved -> Booked (new) + Booked -> Available (old) on reschedule-approve; Reserved -> Available on reschedule-reject; Booked -> Available on cancel-approve | OLD | NEW: Reserved missing | **Add Reserved + transitions** | B |
| Notifications (Email + SMS) per event | OLD | NEW: TO VERIFY | **Subscribe handlers** to ChangeRequestApproved / ChangeRequestRejected events | I |
| Permissions | -- | -- | **`CaseEvaluation.AppointmentChangeRequests.{Default, Approve, Reject}`** -- Staff Supervisor + IT Admin | B |
| Concurrency: two supervisors approving same request | OLD: no guard visible | -- | **Add concurrency stamp** on `AppointmentChangeRequest`; reject second update | I |
