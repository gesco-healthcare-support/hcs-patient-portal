...
| Doctor preferred locations table + toggle UI | OLD entity + UI | NEW: ADDED 2026-05-03 (Phase 7b) | **Add `DoctorPreferredLocation` entity + AppService Toggle** | I | [IMPLEMENTED 2026-05-03 - pending testing] -- `Domain/DoctorPreferredLocations/DoctorPreferredLocation.cs` (composite key `DoctorId + LocationId`, `IsActive` toggle, `IMultiTenant`). `IDoctorPreferredLocationsAppService` exposes `GetByDoctorAsync` (open to authenticated booking flow) + `ToggleAsync` (gated on `DoctorPreferredLocations.Toggle`). Migration `20260504010608_Phase7b_Add_DoctorPreferredLocations`. Mirrors OLD `DoctorPreferredLocationDomain.cs:45-108` upsert pattern. |
