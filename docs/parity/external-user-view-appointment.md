...
| Eager-load full graph | OLD includes 7+ entities (verified `AppointmentDomain.cs`:66 -- Patient/AppointmentAccessors/AppointmentPatientAttorneys/AppointmentDefenseAttorneys/AppointmentInjuryDetails/CustomFieldsValues/AppointmentEmployerDetails + per-injury sub-fetch for PrimaryInsurance/ClaimExaminer/BodyPartDetail) | NEW includes ~5 | [IMPLEMENTED 2026-05-04 - pending testing] (Phase 13b) -- `AppointmentWithNavigationProperties` (Domain) and `AppointmentWithNavigationPropertiesDto` (Contracts) extended with: `AppointmentDefenseAttorney` (1:1), `AppointmentEmployerDetail` (1:1, OLD-parity 1:N noted in audit), `AppointmentInjuryDetails` (List with sub-entities BodyParts / ClaimExaminer / PrimaryInsurance / WcabOffice), `AppointmentAccessors` (List). `EfCoreAppointmentRepository.GetWithNavigationPropertiesAsync` populates each via dedicated private loader methods (LoadDefenseAttorneyAsync / LoadEmployerDetailAsync / LoadInjuryDetailsAsync / LoadAccessorsAsync). Mirrors OLD's per-injury sub-fetch loop (`AppointmentDomain.cs:80-95`). Mapperly mapper picks up the new properties automatically because the per-entity With-Nav mappers already exist. CustomFieldsValues cascade is Phase 6 territory and lands when 6's AppointmentTypeFieldConfigValue feature surfaces a With-Nav DTO. | B |
...
| Internal-only fields (`InternalUserComments`) hidden from external users | TO VERIFY OLD behavior | NEW: `InternalUserComments` exists on `AppointmentDto` exposed to all users | [IMPLEMENTED 2026-05-04 - pending testing] (Phase 13b) -- pure helper `ExternalUserDtoFilter.MaskInternalFields(dto, isExternalUser)` (Application/Appointments/, internal static) nulls `InternalUserComments` when the caller is NOT in any internal role. Wired into `GetAsync`, `GetWithNavigationPropertiesAsync`, `GetByConfirmationNumberAsync` after the ObjectMapper call so both single-row and nav-properties payloads are masked. 7 unit tests cover external-mask / internal-keep / null-passthrough / nav-wrapper / other-fields-untouched. | I |
