...
| Email on upload (to uploader + responsible user) | `SendDocumentEmail` triggered | Phase 18 forward-declared `AppointmentDocumentUploadedEto` exists; per-feature handler not yet wired | **Publish `AppointmentDocumentUploadedEto`** from upload AppService; per-feature email handler lands in Phase 14b follow-up (using Phase 18's `INotificationDispatcher`) | I | `[IMPLEMENTED 2026-05-04 - tested unit-level]` (publish only) / `[DEFERRED to Phase 14b]` (email handler) -- the AppService publishes the Eto on every upload path. The handler that consumes it + dispatches the patient-uploader email + responsible-user email lands in a Phase 14b commit alongside the dispatcher infrastructure. |
| Reminder cadence | Multi-step reminders pre due date | 3 existing recurring jobs (`RequestSchedulingReminderJob`, `CancellationRescheduleReminderJob`, `AppointmentDayReminderJob`) -- none specifically for package docs | **Add `PackageDocumentReminderJob`** | I | `[DEFERRED to Phase 14b]` -- the cadence depends on `Appointment.DueDate` being writable post-creation (Phase 11 territory; verify Session A wires it). Phase 14b commits the reminder job after Session A's manager rewrite settles. |
