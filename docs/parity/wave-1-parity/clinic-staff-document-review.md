...
| Notification on accept/reject | OLD: `EmailTemplate.PatientDocument{Accepted,Rejected}` | Phase 18 declared the Etos; Phase 14 publishes them | -- | -- | `[IMPLEMENTED 2026-05-04 - tested unit-level]` -- Phase 14b ships `DocumentAcceptedEmailHandler` + `DocumentRejectedEmailHandler` subscribing to the Etos. Each dispatches `PatientDocumentAccepted` / `PatientDocumentRejected` template via Phase 18's `INotificationDispatcher`, resolving the original uploader (with patient/booker email fallback when anonymous via verification code). |
| Email subject includes patient + claim + ADJ | OLD pattern | TO REPLICATE in subject builder | **Subject builder helper** that pulls `Patient.FirstName/LastName + InjuryDetail.ClaimNumber + WcabAdj` | C | `[IMPLEMENTED 2026-05-04 - tested unit-level]` -- shared `EmailSubjectBuilder.BuildIdentitySuffix` (Phase 14b, 13 unit tests) returns OLD-verbatim "(Patient: ... - Claim: ... - ADJ: ...)". Surfaces via `##EmailSubjectIdentity##` template variable populated by `DocumentNotificationContext.BuildVariables` (shared bag used by all four document email handlers). |
