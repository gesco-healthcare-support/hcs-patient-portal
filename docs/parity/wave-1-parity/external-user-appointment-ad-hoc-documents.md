...
| Email subject format with patient name + claim # | OLD format: `(...{Patient Name}...{Claim}...{WCAB ADJ}...)` | TO REPLICATE in subject builder | **Add subject builder helper** that includes patient name, claim, ADJ | I | `[IMPLEMENTED 2026-05-04 - tested unit-level]` -- Phase 14b ships `EmailSubjectBuilder.BuildIdentitySuffix(first, last, claim, wcabAdj)` (pure static, 13 unit tests) returning OLD-verbatim "(Patient: {first} {last} - Claim: {claim} - ADJ: {adj})". Surfaces via `##EmailSubjectIdentity##` template variable; the per-feature email handlers populate it through `DocumentNotificationContext.BuildVariables`. |
