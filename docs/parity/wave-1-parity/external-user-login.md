...
| External -> /home, internal -> /dashboard routing | [IMPLEMENTED 2026-05-04 - Phase 9 L7 closeout] -- `postLoginRedirectGuard` (`angular/src/app/shared/auth/post-login-redirect.guard.ts`) wired as `canActivate` on the `/` route. Reads `currentUser` from `ConfigStateService` and returns a `UrlTree` redirect to `/dashboard` when every role on the user is internal (admin / Clinic Staff / Staff Supervisor / IT Admin / Doctor). External users (Patient / Applicant Attorney / Defense Attorney / Claim Examiner / Adjuster) and anonymous users pass through to home. Pure role-classifier extracted to `shared/auth/external-user-roles.ts` (`hasOnlyExternalRoles` / `hasAnyExternalRole`) so the same canonical list backs the guard, the AppComponent's external-user CSS toggle, and any future guard. |
...
| L7 Angular post-login routing guard | [IMPLEMENTED 2026-05-04 - Phase 9 L7 closeout] -- `postLoginRedirectGuard` ships in `angular/src/app/shared/auth/post-login-redirect.guard.ts`; wired on `/` route in `app.routes.ts`. Pure role classifier extracted to `shared/auth/external-user-roles.ts`. |
