...
| Patient dedup field set divergence (NEW finding 2026-05-04) | OLD's 6 fields: LastName / DOB / Phone / Email / SSN / ClaimNumber | NEW's existing `IPatientRepository.FindBestMatchAsync` uses: FirstName / LastName / DOB / SSN / Phone / ZipCode | [IMPLEMENTED 2026-05-04 - pending testing] (Phase 11k + audit closeout) -- new repo method `IPatientRepository.GetDeduplicationCandidatesAsync(tenantId, lastName, dob, phone, email, ssn, claimNumbers)` implements the OLD-parity SQL prefilter (any-of-5-fields OR; ClaimNumber prefilter dropped because NEW's `Patient` does not carry the column -- claim numbers live on `AppointmentInjuryDetail`). AppService wiring landed at `PatientsAppService.GetOrCreatePatientForAppointmentBookingAsync` (audit closeout 2026-05-04, after Phase 11h shipped without it): the OLD-parity 3-of-6 dedup runs after the email fast-path and before the existing `_patientManager.FindOrCreateAsync` safety net. If a candidate matches the threshold, the existing Patient is returned without creating a new row. The legacy `FindBestMatchAsync` stays in place as a non-OLD-parity safety net using NEW's diverged field set. |
...
| AppointmentManager.CreateAsync rewrite -- internal-user fast-path | `AppointmentDomain.cs:221-240` UserType.InternalUser branch | [IMPLEMENTED 2026-05-04 - pending testing] (Phase 11h) -- `BookingFlowRoles.IsInternalUserCaller(roles)` pure helper recognises admin / Clinic Staff / Staff Supervisor / IT Admin / Doctor; `AppointmentsAppService.CreateAppointmentInternalAsync` threads the resulting `AppointmentStatusType` (Approved for internal, Pending for external) into `Manager.CreateAsync`. The slot transition cascades through the existing `SlotCascadeHandler` (Pending->Reserved, Approved->Booked already mapped). 11 boundary unit tests. |
| AppointmentManager.CreateAsync rewrite -- Adjuster auto-fill of ClaimExaminerEmail | `AppointmentDomain.cs:358-380` Adjuster role override | [IMPLEMENTED 2026-05-04 - pending testing] (Phase 11h) -- `BookingFlowRoles.ResolveClaimExaminerEmail(roles, currentEmail, dtoEmail)` pure helper; AppService overrides DTO value with `CurrentUser.Email` when the caller is in the Adjuster role. Mirrors OLD's strict-parity override (the UI rendered the field readonly; this is the server-side belt-and-suspenders). |
| AppointmentManager.CreateAsync rewrite -- sub-entity attachment via DTO collections | `AppointmentDomain.cs` Add path step 7 | [DESCOPED 2026-05-04 - intentional architecture deviation] -- OLD posts the entire intake (appointment + injuries + employers + accessors + attorneys + custom fields) in one POST. NEW's existing pattern is multi-step: POST /api/app/appointments to create the parent, then POST /api/app/appointment-injury-details, /api/app/appointment-employer-details, /api/app/appointment-accessors, /api/app/appointments/{id}/applicant-attorney, /api/app/appointments/{id}/defense-attorney to attach sub-entities. The functional outcome is identical; the choreography differs. Adding sub-entity collections to AppointmentCreateDto would be a NEW deviation from NEW's own pattern; preserving NEW's per-feature endpoints is the cleaner choice. Angular booking form already calls the per-feature endpoints sequentially. The Phase 11i `AppointmentAccessorManager.CreateOrLinkAsync` is reachable through the AppointmentAccessorsAppService path, which the booking form invokes once per accessor. |
...
| `IAppointmentAccessorManager.CreateOrLinkAsync` | `AppointmentAccessorDomain.cs` | [IMPLEMENTED 2026-05-04 - pending testing] (Phase 11i) -- `AppointmentAccessorManager.CreateOrLinkAsync(appointmentId, email, requestedRoleName, accessTypeId, tenantId)` orchestrates the OLD-parity Add path (lines 186-196 + 222 / 290+). Pure decision logic in `AppointmentAccessorRules.ResolveOutcome` (4 outcomes: CreateUserAndLink / LinkExisting / RoleMismatch / GrantRoleAndLink). Side-effects: creates IdentityUser via `IdentityUserManager.CreateAsync` with random temp password, ensures role exists via `IIdentityRoleRepository`, grants role via `AddToRoleAsync`, persists `AppointmentAccessor` link via `CreateAsync`, publishes `AppointmentAccessorInvitedEto` for the invitation email. RoleMismatch path throws `BusinessException(AppointmentAccessorRoleMismatch)` with the verbatim OLD message ("Your added accessor '{0}' is already registered in our system with different user type. Please select proper Accessor user's type and try again"). 16 unit tests cover the pure rule branches; orchestrator integration tests come with Phase 20 E2E (Identity stack). AppService wiring is part of Phase 11h. |
| `CloneForRescheduleAsync` cascade-copy (11.2) | `AppointmentChangeRequestDomain.cs` | [IMPLEMENTED 2026-05-04 - pending testing] (Phase 11c + 11j) -- Phase 11c shipped `BuildScalarClone` for the appointment row itself. Phase 11j added per-child-entity pure clone helpers in the same `AppointmentRescheduleCloner` static class: `CloneInjuryDetailFor`, `CloneBodyPartFor`, `CloneClaimExaminerFor`, `ClonePrimaryInsuranceFor`, `CloneEmployerDetailFor`, `CloneApplicantAttorneyFor`, `CloneDefenseAttorneyFor`, `CloneAccessorFor`. Each helper preserves every scalar field (back-fills the post-construction ones), generates a fresh Id, re-points the parent FK, and assigns the supplied tenant. 10 unit tests cover the per-entity branches + null-source throws. The orchestrator (fetch-by-AppointmentId + clone + insert per child collection) lives in Phase 17's change-request approval AppService since it is the only consumer; the helpers ship now so 17's commit is purely orchestration. AppointmentTypeFieldConfigValue + AppointmentDocument cascade is deferred to Phase 14 (where the document-specific Status preservation rule applies); those entities remain unchanged here. |
