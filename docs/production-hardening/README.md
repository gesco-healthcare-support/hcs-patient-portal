...
| 3 Critical-path coverage | **IN PROGRESS -- 3.1 to 3.4 COMPLETE; 3.5 CHARACTERIZED.** Task 9 CLOSED, both halves. Task 6.2 blocked by design (#710). 3.5's guarantee cannot be covered in the current harness -- tracked as #732 | #686 `a8e221df`, #688 `d1d70938`, #689 `c9ad7de8`, #693 `3c1e5d6f`, #697 `753e7585`, #705 `4a2fd03d`, #709 `28dadced`, #711, #716 `eb3850c7`, #725 `ad3dd00e` | Tenancy went from **0 of 323** backend test files asserting the resolver chain to pinned in BOTH processes, every test seen to fail. `FLOOR_BACKEND` 73 -> 72, honestly measured. **A REAL access-control defect found and fixed**: all four `AppointmentPacketsAppService` methods took a caller-supplied appointment id with no party check. Packet-kind filtering could be deleted entirely with zero of 1,925 tests failing. A booted application carries exactly one ABP default resolver and the harness seeds none, so `Clear()` removes something real -- the fact a bare `ServiceCollection` could not exhibit. With `Clear()` removed the framework's resolver is still present and ours appended AFTER it, which CONFIRMS the ordering rather than merely detecting a change. **3.5 found a guardian that does not guard**: two tests named for booking atomicity are rejected at the DTO boundary before the service runs, and the harness cannot roll back a single row, so no test here can cover it. |
...
| 8   | [Coverage expansion](08-coverage-expansion.md)              | 37.1% (`main`) -> as high as reached | Open-ended and UNTARGETED by Adrian's ruling of 2026-09-08. Degrades gracefully because phase 2 stops backsliding. |
