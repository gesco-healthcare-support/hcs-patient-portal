...
| 3 Critical-path coverage | **IN PROGRESS -- 3.1 to 3.4 COMPLETE, 3.5 remains.** Task 6.2 blocked by design (#710) | #686 `a8e221df`, #688 `d1d70938`, #689 `c9ad7de8`, #693 `3c1e5d6f`, #697 `753e7585`, #705 `4a2fd03d`, #709 `28dadced`, #711 | Tenancy went from **0 of 323** backend test files asserting the resolver chain to pinned in BOTH processes, every test seen to fail. `FLOOR_BACKEND` 73 -> 72, honestly measured. **A REAL access-control defect found and fixed**: all four `AppointmentPacketsAppService` methods took a caller-supplied appointment id with no party check. Packet-kind filtering could be deleted entirely with zero of 1,925 tests failing. |
