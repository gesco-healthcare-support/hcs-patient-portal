...
| 3 Critical-path coverage | **COMPLETE 2026-09-08** -- all 9 tasks on `main` | #686 `a8e221df`, #688 `d1d70938`, #689 `c9ad7de8`, #693 `3c1e5d6f`, #697 `753e7585`, #705 `4a2fd03d`, #709 `28dadced`, #711, #716 `eb3850c7`, #725 `ad3dd00e`, #734 `30cde971` | Tenancy went from **0 of 323** backend test files asserting the resolver chain to pinned in BOTH processes. **A REAL access-control defect found and fixed**: all four `AppointmentPacketsAppService` methods took a caller-supplied appointment id with no party check. 3.5 found a guardian that does not guard -- two tests named for booking atomicity never exercised it, and the harness cannot roll back a single row, tracked in #732. |
| 4 CodeQL sensitive-info  | **COMPLETE 2026-09-09** -- 0 open, 1 real defect of 19 alerts | #752 `d9a64b20`, #755 `dea8138f`, #761 `2e0f3e11`, cascade #768 | A full patient email address reached a Warning-level log; masked at the construction site. **The fix did NOT close the alert** -- CodeQL cannot model the sanitizer, so it was dismissed Won't fix and a permanent model is tracked in #772. Two genuine WCAG AA failures were found by reading the neighbouring code (#744, #756). The done-bar also had an unstated dependency on a weekly job. |
| 5 Security hotspots      | **COMPLETE 2026-09-08** -- 0 TO_REVIEW, 53 REVIEWED/SAFE | #722 `770e4478`, #724 `0a8a9917`, #728 `bab4d08b` | Done in a parallel session. `add-worktree.sh` was not merely running lifecycle scripts -- it had been **failing outright** since the Yarn 4 move, roughly three months, and the remedy the issue prescribed (`--ignore-scripts`) does not exist in Yarn 4. |
| 6 Dependencies           | **DEFERRED 2026-09-09** until coverage is raised -- 6.1 #585, 6.2 #586, 6.3 #587 all OPEN; 6.4 #699 CLOSED 2026-09-15 | -- | 99 advisories live, not the 88 previously recorded. **27 sit on packages that ALREADY carry a `resolutions` pin**, and two of those pins are exact versions that are themselves vulnerable -- holding ten alerts open. Research recorded on #585. |
| 7 Rule families          | NOT STARTED                                                   | --                                                                                      | **RE-DERIVE THE SIZE BEFORE PLANNING.** This phase records 1,249 issues across 100 families from 2026-08-31; SonarCloud reports **398 across 64 rules** on `main` @ `e6b4093d` (2026-09-16). The code-cleanup campaign has been closing findings all month, so the two figures may differ by scope -- but nothing should be planned off the older one. No `Hardening 7.*` issue exists. |
| 8 Coverage expansion     | **IN PROGRESS 2026-09-16** -- 8.1 settled, **8.1b COMPLETE**, tranche 1 running | #777 `2abe0699`; 8.1b #784/#785/#786/#787 + #807 all CLOSED; tranche-1 product fixes #863, #906, #913 all MERGED | **46.5%**, 13,815 uncovered of 27,682 (`main` @ `e6b4093d`, 2026-09-16). Back end 56.5% (7,442 uncovered), Angular 36.3% (5,415). Target is 90% overall AND 90% per module. 8.1 settled the denominator: 1,300 Python lines stay IN because `tools/packet-templates` and `docker/packet-renderer` ship inside the production image (`Dockerfile:4`, `:60`). Open follow-ups: **#911, #912**. **Tranche 1 is 7 EXTEND-broad files totalling exactly 1,882 uncovered**, grouped into 5 tasks; task 1 in progress. The C# and Angular bulk is ~91% of what remains and **is still tracked in no issue** -- the largest gap in this tracker. |
| 9 System design intake   | **TRIAGE COMPLETE, routing done**                             | report received 2026-08-31                                                              | 4 claims refuted, 3 confirmed. 62 capability requirements, 10 application-owned, 17 anti-requirements. Findings routed into phases 1, 2, 3 and 8; the remainder sit on this phase's own deferred list. No further work scheduled. |
...
(PR #671) on 2026-09-04.**

> **Corrected 2026-09-16.** This said "#672 stands and is actionable". **#672 is now CLOSED.** The
> re-import it describes is done or abandoned -- check the issue before acting on the procedure
> above. Catalogue instance: a record entry correct when written and wrong after the work moved,
> still reading plausibly. The counts in the table above were measured 2026-09-04 and should be
...
87 npm packages against 37.1% coverage is exactly the scenario Adrian is worried about. **That figure
read 52.4% when this paragraph was written and the argument has since got STRONGER, not weaker** --
see the correction under Baseline measurements.
...
| 4   | [CodeQL sensitive information](04-codeql-sensitive-info.md) | 19 alerts, **1 real** | Highest value per unit on a PHI system |
| 5   | [Security hotspots](05-security-hotspots.md)                | 27 reviewed, **0 remaining** | Review-and-decide, not necessarily fix |
| 6   | [Dependencies](06-dependencies.md)                          | **99 advisories**, 1 critical | Deferred until coverage rises. 3 of 4 items open (#585, #586, #587); #699 closed. |
| 7   | [Rule families](07-rule-families.md)                        | 1,249 in 100 families (2026-08-31); **398 / 64 rules measured 2026-09-16** | Bulk. Largest families first; 72% sits in the top 15. **Re-derive the count before planning.**                     |
| 8   | [Coverage expansion](08-coverage-expansion.md)              | **46.5%** (`main` @ `e6b4093d`, 2026-09-16) | 13,815 uncovered. Back end 56.5%, Angular 36.3%. Moves daily -- re-measure, do not cite |
...
> **The Coverage and Lines-of-code rows are SUPERSEDED as current figures, 2026-09-08.** They were
> correct on 2026-08-31 and are kept unedited because this table is dated. `main` now reports
> **coverage 37.1%** (line 41.0%, branch 17.5%) across **117,310 lines**.
>
> **The drop is not a regression.** Item 2.13 made 184 previously invisible Angular sources count
> toward the denominator, so the front end fell 69.46% -> 20.97% and the blend followed. Same tests,
> same passes, same code covered. **Anyone reading 52.4% -> 37.1% as falling quality has it exactly
> backwards**, and anyone sizing phase 8 against the row above will be fifteen points optimistic.
> [08-coverage-expansion.md](08-coverage-expansion.md) carries the split, the branch-coverage figure
> and what 90% would cost.
>
> **This is catalogue instance 29's class again** -- a record entry correct when written and wrong
> once our own work changed what it described, still reading plausibly. It survived four days and was
> found only because Adrian asked which phase closes the coverage gap. **When an item closes, grep
> this folder for entries that argue from the figure it moved.**
