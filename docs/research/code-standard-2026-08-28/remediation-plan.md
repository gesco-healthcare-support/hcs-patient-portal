...
>
> **CAPACITY CAVEAT (added 2026-08-28).** The 40 h/month figure originated as a working
> assumption in the research brief, not as a stated allocation. The per-item hour estimates are
> independent of it and stand on their own; every conversion to calendar time below depends on
> it. Do not quote a "months" figure until the allocation is confirmed.
>
> **REPOSITORY VERIFICATION (2026-08-28).** This plan was produced by a session with web access
> but no repository, so every statement about current state was inherited from baseline
> documents. The checks it asked for have since been run against source, and the affected items
> below are corrected in place and marked. Items 1, 2, 7, 8, 10, 12, 13, 14, 18, 23, 24, 28 and
> 29 changed; a new item 0 was added; the revised launch-blocking total is **~135 h**, not 141.
...
Revised 2026-08-28 after repository verification. The "At 40 h/month" column is retained only
because it is what the original analysis said; see the capacity caveat above before quoting any
calendar figure.
...
| Do soon (first month after launch) | 62 | 62 | 1.5 months |
| Do eventually | 78 | 78 | 2 months |
...
pipeline. Today the merge gate does not consider the tests, and three secret scanners fail open.
That is not a "some gaps remain" position; it is a pipeline that reports success without checking.

> **Corrected 2026-08-28.** Two claims here were softened after verification. (a) The tests **do**
> run on every PR -- `ci.yml:127` `backend-test` and `ci.yml:216` `frontend-test`, neither masked --
> they are simply not *required checks*, so a red test does not block a merge. The gap is the gate,
> not the tests. (b) The only real SAST **is** running: CodeQL's `javascript-typescript` analysis
> completes and uploads SARIF, so its zero alerts are a real result, not a broken configuration.
...
| # | Item | Effort | Depends on | Launch-blocking |
| --- | --- | --- | --- | --- |
| 1 | **CORRECTED TARGET 2026-08-28:** `.husky/` does not exist. `core.hooksPath` is `angular/.husky/_`, so the files are `angular/.husky/pre-commit` (the else branch at 16-20, and the `node_modules` conditional at 23) and `angular/.husky/pre-push` (9-19). A third fail-open path the analysis could not see: `core.hooksPath` is LOCAL git config, so a fresh clone runs no hooks at all until husky install has run -- the durable control is CI-side, not the hook. **Hard-fail the gitleaks hooks.** Replace the silent-degradation check in `angular/.husky/pre-commit` and `angular/.husky/pre-push` with `command -v gitleaks >/dev/null \|\| { echo 'gitleaks not installed'; exit 1; }`. Same treatment for the `lint-staged only if node_modules exists` conditional, which fails open identically. Delete the pre-push full-tree scan -- it rescans unchanged history on every push and is what drives people to `--no-verify` | 2 h | -- | **YES** |
| 2 | **CORRECTED 2026-08-28 -- this does NOT fix the CVEs.** The analysis called it the highest value-per-minute change on the theory that `target-branch` suppressed security updates. It did not: PR #417 and #416 both carry `baseRefName: main` and were closed by the maintainer on 2026-08-07 with every check green. Dependabot then replied it will not raise that release again, so 20.3.x is silently untracked and must be bumped by hand (item 18). This item fixes version-update routing only. **Delete `target-branch` from `dependabot.yml`.** Keep `open-pull-requests-limit: 0`; it is correct and does not affect security updates. Confirm security updates are enabled in repo Settings (a repo-level toggle, not a config key) | 0.5 h | -- | **YES** |
...
| 7 | **REFUTED 2026-08-28 -- do not spend the two hours.** `HasFilter` IS in the model configuration: `CaseEvaluationDbContext.cs:151,170` plus 8+ sites in `CaseEvaluationSharedModelConfiguration.cs` (19 in the migration, 20 in the snapshot). Live hypothesis is now SQLite vs SQL Server filtered-index fidelity -- the failing index at `CaseEvaluationSharedModelConfiguration.cs:446-448` IS filtered, yet SQLite reports a bare unique violation. Confirm under item 20; if it passes on SQL Server and fails on SQLite, close this as evidence FOR item 20 rather than as a separate defect. **Diagnose the flaky test properly.** Check whether `Fix_UniqueIndexesExcludeSoftDeleted` put `HasFilter` in `OnModelCreating` or only in the migration file. Do **not** relax the SQLite assertion -- SQLite supports UNIQUE partial indexes and EF Core emits the filter, so the failure is a true-positive drift signal | 0.5 h | 20 | **YES** |

### Week 2: build the gate that does not exist (15 h)

| # | Item | Effort | Depends on | Launch-blocking |
| --- | --- | --- | --- | --- |
| 8 | **CORRECTED 2026-08-28: the two test jobs ALREADY EXIST and already run** -- `ci.yml:127` `backend-test` (`dotnet test` at `:161`) and `ci.yml:216` `frontend-test`, neither carrying `continue-on-error`. They go red correctly; they are simply not *required checks*. Only the aggregator is missing, so this is 3 h, not 8. Note `backend-format` and `frontend-format` DO carry `continue-on-error` (`:266`, `:301`), so including them in `needs` proves nothing. **Move `dotnet test` into `ci.yml` and build the aggregator.** Add `backend-test` and `frontend-test` jobs, then a `ci / gate` job with `needs: [backend-build, backend-test, frontend-build, frontend-test, secrets, deps-audit]` and `if: always()`, failing unless every needed result is success or skipped. Add `tenant-isolation` and `workflow-audit` to `needs` as those land | 3 h | 4 | **YES** |
...
| # | Item | Effort | Depends on | Launch-blocking |
| --- | --- | --- | --- | --- |
| 12 | **FALSE POSITIVE -- verified 2026-08-28, do not spend the four hours.** Traced by hand: the target is built from the `ADMIN_SLUG` constant and `baseHost` from `dynamic-env.json` (our own config). `window.location.hostname` is only COMPARED to decide whether to redirect, never interpolated into the target; path and query land after the authority is fixed and cannot alter the host. Dismiss `tssecurity:S6105` in SonarCloud with that justification. **Read `angular/src/tenant-bootstrap.ts:65`** (`tssecurity:S6105`) before anything else on this list. A taint-traced redirect from user-controlled data, on the tenancy resolution path of a Host-header-tenanted PHI app, days from public exposure. Trace the value to its source by hand. Fix with a server-side allow-list of the 11 office hosts, not URL sanitisation | 0.5 h | -- | **YES** |
| 13 | **MOSTLY ALREADY DONE -- verified 2026-08-28.** Part (a) is satisfied: all four user seed contributors (`InternalUsers`, `ExternalUsers`, `DemoExternalUsers`, `DemoPatient`) call a private `IsDevelopment()` reading `ASPNETCORE_ENVIRONMENT` then `DOTNET_ENVIRONMENT`, and FAIL CLOSED -- unset or misspelled skips them. Only part (b) remains, and only **2 databases exist**, not 11. Residual risk: the default password is a public const in a PUBLIC repo at `InternalUsersDataSeedContributor.cs:35` and `ExternalUsersDataSeedContributor.cs:41`. `SetShouldChangePasswordOnNextLogin` does not stop the FIRST login being an attacker, so rotation is the control. **Audit the seed contributors.** Two separate things: (a) confirm `Demo*` contributors cannot execute outside Development -- the robust fix is not compiling them into the deployed artifact, since ABP seeding runs in production via DbMigrator on every deploy *and* for every new office; (b) query all 11 tenant databases and confirm what admin password each actually holds. Rotate anything default, and make the seed path require the value from configuration with no literal fallback. **Free now, expensive after go-live** | 1.5 h | -- | **YES** |
| 14 | **FALSE POSITIVE -- verified 2026-08-28.** The dismissal condition this item itself sets is already met: the `name` input is typed as `IconName` and resolved against the static `ICON_PATHS` map (empty string for unknown keys), `size` is a number, and `label` is HTML-escaped by a private `escape()` one line before interpolation. No external text reaches the markup. Dismiss `typescript:S6268`. **Read `shared/ui/icon/icon.component.ts:58`** (`typescript:S6268`, sanitisation disabled). If the input is a literal union resolved against a static map, dismiss and narrow the type so it stays that way. If it is `string`, it is a reflected XSS primitive on every page including anonymous login | 0.25 h | -- | **YES** |
...
| # | Item | Effort | Depends on | Launch-blocking |
| --- | --- | --- | --- | --- |
| 18 | **SSR QUESTION SETTLED 2026-08-28: there is no SSR anywhere** -- no `@angular/ssr`, no server entry, no `provideClientHydration`, no ssr/prerender builder target -- so four of the CVEs are not exploitable here and this is hygiene rather than an open wound. Dependabot will NOT re-raise 20.3.x (see item 2), so the bump is manual. Confirm ABP Commercial 10.0.2 peer ranges before assuming a target version installs. **Settle the SSR question, then bump Angular.** Fifteen minutes of greps (`@angular/ssr`, `server.ts`, `provideClientHydration`, an `ssr`/`prerender` builder target) determines whether four of the CVEs are even exploitable here. Then one atomic PR moving all `@angular/*` to **20.3.30** (current `v20-lts`, not the 20.3.27 in the closed PRs). Replace the blanket version-update disable with an `ignore` rule scoped to `version-update:semver-major`. Smoke: login, password reset, upload, and tenant resolution across two office hostnames | 3.5 h | 2 | **YES** |
...
| 23 | **BIGGER THAN WRITTEN -- verified 2026-08-28.** The limiter is configured in exactly one place, `CaseEvaluationHttpApiHostModule.cs:628`, covering password-reset, upload-by-code, `/api/public/external-signup/register` and `/api/integration`. **Sign-in is served by the AuthServer, a separate process that registers no rate limiter at all.** The AuthServer also calls app services in-process, so API middleware never runs -- the 2026-08-22 password-reset fix had to move the throttle into the shared service for exactly this reason (see the comment at `:664-669`), and registration still has that shape. Behind a reverse proxy, per-IP partitioning needs correct `UseForwardedHeaders` or every request partitions to the proxy. **Rate-limit login and registration.** Today only password reset, public upload and external-account endpoints are throttled. Login and registration are the two endpoints anonymous internet traffic will hit hardest | 5 h | -- | **YES** |
| 24 | **LOCATED 2026-08-28.** The dead wildcard is `angular/nginx.conf:30` on `location = /getEnvConfig`, which serves public boot config -- low risk in itself. The real half is that both CORS policies call `SetIsOriginAllowedToAllowWildcardSubdomains()` AND `AllowCredentials()` together (`CaseEvaluationHttpApiHostModule.cs:1233-1235`, `CaseEvaluationAuthServerModule.cs:410-420`). There is no `AllowAnyOrigin()` and no literal wildcard origin anywhere in `src/`. **Delete the dead wildcard-CORS endpoint** -- the endpoint, not just the header. Then grep for other `*` CORS values and confirm the real API uses an explicit per-tenant origin allow-list, with a test. Given Host-header tenancy that allow-list is load-bearing | 4 h | -- | **YES** |
...
| 28 | **CONFIRMED 2026-08-28, and more specific than written.** Four of the eight are CRITICAL `typescript:S2871` (`.sort()` with no comparator) at `gen-slots.util.ts:103`, `internal-generate-slots.component.ts:224` and `external-home.component.ts:343,346`. **Two are in slot generation**, where lexicographic sorting puts "10:00" before "9:00" -- a plausible LIVE user-facing defect on the booking screen. Do this early. **Find the 8 bugs behind the reliability D.** One SonarCloud filter: Reliability + High/Blocker. The 330 accessibility findings contribute nothing to the rating; these 8 contribute all of it. A CRITICAL C# bug in a PHI backend is usually a null-deref, a disposed-object use, an `async void` or a swallowed exception on a data path | 4 h | -- | **YES** |
| 29 | **ONE SUB-TASK DROPPED 2026-08-28: do not delete `labeler.yml`.** Both `pull_request_target` workflows (`labeler.yml`, `pr-title.yml`) do NOT check out PR code, which is the safe pattern and the entire reason that trigger is dangerous. The rest of this item stands; note `ci.yml:21` already sets `permissions: contents: read`. **Pipeline security pass.** Add `permissions: contents: read` at the top of every workflow with narrow job-level grants; check Settings > Actions > General is on the restricted default (if the org predates February 2023 the read-only default was never applied); pin third-party actions to full commit SHAs; add zizmor + actionlint as one `workflow-audit` job scoped to `.github/workflows/**`; delete `labeler.yml` and its `pull_request_target` trigger | 7.5 h | 8 | **YES** |
