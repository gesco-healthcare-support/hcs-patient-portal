...
**T4/T5 tension resolved 2026-04-24 during T7.** Adrian confirmed during the T7 AppointmentEmployerDetails interview that **ANY** data captured on a submitted appointment form -- patient, attorneys, insurance + adjustor, employer, etc. -- is locked at submit (because the all-parties notification email that fires on submit is a legal record), and any post-submit change requires Gesco-side admins running the proper process. Reconciliation (a) wins: the T4 claim that the practice-side doctor's admin can make form-data edits applies ONLY pre-submit. After the request is submitted and notifications have fired, the practice-side admin cannot change form data; it's Gesco-side only. The T4 personas description in `appointments.md` should be read as "form-data edits" meaning pre-submit corrections during the booker's drafting phase, not post-submit changes. This universal rule is further captured in `appointment-employer-details.md` and applies retroactively to all T4-T6 intent. [Source: Adrian-confirmed 2026-04-24 during T7]
