...
- 2026-04-27 (T13 Tenant-provisioning cross-cutting) -- new file `docs/product/cross-cutting/tenant-provisioning.md`. Resolutions in session via Adrian interview: (a) **Onboarding form supports two modes** (T13-Q1) -- host admin can either fill in everything in one go (full-data path) OR create a bare-bones tenant with just the minimum (Doctor + Practice Admin user + supervisor admin) and send a magic-link invite for the Practice Admin to complete setup post-login. Both modes are first-class. (b) **Tenant-live criteria** (T13-Q2) -- the full-data path's all-required is the default; the bare-bones path's tenant becomes implicitly live once the missing data (Locations + AppointmentType coverage) is added by the Practice Admin post-login. Criteria are: at least one Location + at least one AppointmentType in coverage + at least one Practice Admin user + supervisor admin assigned + Doctor record present. (c) **Magic-link pattern reused** for the initial Practice Admin's first login (consistent with the T10 Q-T10-4 Patient auto-create pattern -- one-time link, sets password during first session). (d) **Tenant URL / access model** (T13-Q3) -- Adrian deferred; the question needs to be reframed in non-technical business-shape language for the manager (per the `feedback_business_questions_not_architecture_questions` memory). T13 keeps the technical version in its Outstanding Questions; the email-craft step (next on the run sheet) creates the manager-facing version. The cross-cutting file consolidates T0 tenant-unit + T4 host-admin-manual onboarding (Q20) + T8 initial-Locations seeding (Q-A3) + T9 decommission (Q-T9-4) + T10 Practice-Admin + supervisor-admin assignment (Q-T10-3 / Q-T10-6). New code gaps captured in Known Discrepancies on the cross-cutting file: DoctorTenantAppService onboarding fields likely diverge from the two-path intent; magic-link Practice-Admin invite unbuilt; setup-incomplete tenant state unbuilt; setup-checklist Practice-Admin first-login UI unbuilt; supervisor-admin role missing in code (T10 Known Discrepancy); tenant-archive-in-place state unbuilt (T9 Known Discrepancy); tenant-context audit-logging unbuilt. Inline-seeded with NEEDS-CONFIRMATION on review: bare-bones bounce handling; setup-incomplete tenant visibility in tenant picker; doctor-comeback / re-activation pattern (assumed not-at-MVP). One new manager-bound area to draft for the upcoming email round: **tenant URL / access model**.
- 2026-04-27 (T12 Notifications cross-cutting) -- new file `docs/product/cross-cutting/notifications.md`. Resolutions in session via Adrian interview: (a) **Recipient list per event** (T12-Q1) -- one canonical all-parties list, identical content per email at MVP; everyone on the case sees the same email content for a given event. Per-event content can vary (different templates per event); per-recipient content does not. Post-MVP candidate: split into primary-vs-CC variants. (b) **Channels at MVP** (T12-Q2) -- email + in-portal inbox copy. Email is the legally-defensible channel; the in-portal inbox is the UX channel. Recipients without portal accounts get email only. (c) **Language at MVP** (T12-Q3) -- all English regardless of patient preferred language; "interpreter required: <language>" call-out on every email when patient language != English. Per-language templates land post-MVP. The cross-cutting file consolidates the all-parties model from T2 + T7 + T8 + T11 + T10 (magic-link Patient invite is a separate notification type). New code gaps captured in Known Discrepancies on the cross-cutting file: email sender / template system unbuilt (FEAT-05); no in-portal inbox in Angular; no translator-required call-out logic; no magic-link auth flow; no all-parties recipient compute; no per-recipient delivery-status tracking; no retry / bounce handling. Inline-seeded with NEEDS-CONFIRMATION on review: sender identity at MVP (generic Gesco vs per-tenant); bounce / retry shape; reminder / digest emails (assumed not-at-MVP per portal-is-booking-only). No new manager-bound questions added in T12; format specifics remain open via OUTSTANDING-QUESTIONS.md Q9 + `escalations/open-items.md` Item 2; Q23 invite-fire timing still open in the manager queue.
- 2026-04-27 (T11 Appointment-lifecycle cross-cutting) -- new file `docs/product/cross-cutting/appointment-lifecycle.md`. Resolutions in session via Adrian interview: (a) **14th state name** (T11-Q1) -- `AwaitingMoreInfo` confirmed for the send-back-for-info status. The 13-value enum in code is incomplete relative to MVP intent; adding the 14th value is a build item. (b) **AwaitingMoreInfo response flow** (T11-Q2) -- when the booker re-submits, the appointment auto-transitions back to `Pending`; the office's review-queue item carries a "response received from booker on previous send-back" flag so they can distinguish a returning request from a fresh one. (c) **Two-enum design** (T11-Q3 + clarification) -- the slot status (3 values: `Available` / `Reserved` / `Booked`) and the appointment status (14 values) stay separate. They synchronise via business rules at key transitions but are not collapsed. **No `Reserved` state on the appointment side** -- `Pending` and `AwaitingMoreInfo` already cover the "request submitted, not yet decided" reality. (d) **Reschedule shape** (T11-Q4) -- admin-initiated reschedule produces a new appointment record (parent-FK link to the original; new global confirmation number per T9 Q-T9-1); the original moves to `RescheduledNoBill` / `RescheduledLate` (terminal on that record). (e) **Send-back content shape** (T11-Q5) -- the office can attach BOTH structured field-flags (specific form fields the office wants the booker to revisit) AND a free-text note. Closes the [UNKNOWN] flagged in `appointments.md` since T2. **Resolves research Q1** (appointment status workflow rules) on the Adrian-direct portion -- the canonical state-machine, the slot-appointment synchronisation table, and the per-transition role-authority table live in T11. Manager confirmation may still narrow specific role-authority rows. New code gaps captured in Known Discrepancies on the cross-cutting file: AwaitingMoreInfo enum value missing; no state-machine enforcement; no transition endpoints; send-back schema unbuilt; reschedule-as-new-record unbuilt; AwaitingMoreInfo timeout job absent; all-parties-notification-on-transition belongs to T12. New Adrian-direct outstanding items captured in the file (not new manager-bound questions): AwaitingMoreInfo timeout duration; field-lock scope during AwaitingMoreInfo (only flagged fields editable, or any field?); reschedule of a still-`Pending` appointment; whether the new appointment record after reschedule starts at `Pending` or `Approved`; send-back history retention. The cancel / reschedule MVP-existence question itself remains in `escalations/open-items.md` Item 1; this T11 doc is provisional on that scope question.
