...
> a public deployment must close, and the questions only Gesco can answer.
>
...
A workers' compensation Independent Medical Examination (IME) scheduling platform. Gesco staff
...
**Who uses it**: internal Gesco staff (IT admin, supervisors, intake) plus four external party
...
| Item          | Value                                                                                |
| ------------- | ------------------------------------------------------------------------------------ |
| Machine       | `appoint-portal`, Ubuntu 24.04.4 LTS, x86_64                                         |
| Resources     | **4 vCPU / 16 GB RAM / 48 GB disk**                                                  |
| Disk headroom | ~9.7 GB free (79% used) after the 2026-08-25 image build                             |
| Addresses     | LAN `192.168.101.37`; VPN `172.25.83.82` (VPN does not route to the LAN subnet)      |
| Runtime       | Docker Engine 29.6.1 + compose plugin v5.3.1                                         |
| App path      | `/home/apadmin/hcs-patient-portal`                                                   |
| Access        | `ssh apportal` (SSH config alias), key `~/.ssh/ap_deploy_ed25519`, passwordless sudo |
...
`pfd.tbc.local` is the **corporate Active Directory DNS zone**. Consequences:
...
- **No public CA can issue a certificate for it.** The current certificate is issued by the
  internal `pfd-Root-CA` (CN `appointment-portal.pfd.tbc.local`, 4 SANs = base plus three
...
- **Mail**: 7 `SMTP_*` variables (currently `mail.securemailprotocol.com:587` STARTTLS)
...
Application-tier rate limiting exists but is narrow -- `ConfigurePasswordResetRateLimiter`
(`CaseEvaluationHttpApiHostModule.cs:625`) covers password reset, public document upload, and
external-account endpoints only. Login, registration, and every business endpoint are
unthrottled.
...
3. **Only two databases exist in production today** -- host plus `CaseEvaluation_falkinstein`.
...
## 7. Questions only Gesco can answer
...
4. Which providers will Gesco sign a BAA with? Is there an existing cloud relationship
...
12. Does Gesco IT participate in the public deployment, or is it wholly the dev's remit? IT
