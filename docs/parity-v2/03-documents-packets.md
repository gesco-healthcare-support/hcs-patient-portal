...
| E3 | Auto-queue package docs on approve | `AddAppointmentDocumentsAndSendDocumentToEmail` fired on approval | `PackageDocumentQueueHandler` on `AppointmentApprovedEto` -> `CreateQueuedAsync` (Pending rows, one per active `DocumentPackage` for the type) | Same outcome: pending rows materialized at approval. |
