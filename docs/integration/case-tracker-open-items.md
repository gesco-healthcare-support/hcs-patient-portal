...
| I7 | Issue `CaseTracker:IntegrationToken`                                                                         | **DONE on our side (verified live 2026-09-04).** The token is set in `secrets/env.prod`, the api container carries it, and the endpoint authenticates: no token and a wrong token both 401, the correct token reaches the handler. What remains is the Case Tracker configuring the matching header. This row previously said "still EMPTY in production", which was wrong and cost a reader a false diagnosis. **UPDATED 2026-09-08: the remaining work is NOT their token config.** They read their production server: `portal.reconcile.token` is set, 64 chars, and their startup log has confirmed it since 2026-08-17. Across 2,565 attendance attempts since 2026-08-19 they have never once received a 401, so they ARE authenticating against this token. The live blocker is instead that every one of those attempts returns non-2xx from us and their 96-hour give-up window never fires, so a real No Show from 2026-08-19 has retried silently for twenty days. See BUG-046 and issue #694 |
