...
| `unit`               | string | Yes                      | ADDED 2026-08-08. Apartment / suite number. SOURCE CORRECTED 2026-08-13 - see the note below.  |
...
- `unit` is the apartment / suite number, the field every portal screen labels **"Unit #"**.
...
**`unit` correction 2026-08-13, worth one paragraph because it changes what you can trust.** Until
this release the unit was stored in two different places depending on which screen captured it, and
we sent only one of them -- the one our staff never write to. The effect was that a unit entered when
the appointment was booked reached you, and a unit later CORRECTED by our staff did not; you kept
receiving the original. Both paths now write the same field and we send that, so a correction reaches
you like any other demographic change.

Two consequences. A `unit` you already hold for an older appointment may be a value our staff have
since corrected, so treat the next push for that appointment as authoritative rather than assuming
nothing changed. And appointments booked before this release that never had a unit recorded will
continue to send none -- we did not retro-fit historical records, because distinguishing a genuine
unit from a mis-keyed street line is guesswork and this data ends up in proof-of-service documents.
