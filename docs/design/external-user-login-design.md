...
| 2 | Single-session enforcement | Delete all prior tokens on login | RESTORED 2026-06-01: a fresh login revokes the user's prior refresh tokens (`RevokePreviousSessionsHandler`); the old device drops at its next silent refresh, within the shortened 15-min access-token lifetime | Prior "acceptable deviation" reversed. Self-contained JWTs are validated locally at the API (no store lookup), so an instant kill is not possible without reworking the validation pipeline; this is the "one durable session" variant |
