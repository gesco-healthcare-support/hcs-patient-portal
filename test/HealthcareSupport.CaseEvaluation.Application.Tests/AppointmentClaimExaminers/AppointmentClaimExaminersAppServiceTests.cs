...
/// <summary>
/// Covers <see cref="IAppointmentClaimExaminersAppService"/>. Before this tranche the only test
/// touching this area was ClaimPartiesDtoMapperUnitTests, which asserts a mapper rather than the
/// service; the 34.8% reported was incidental execution.
///
/// The entity is IMultiTenant and hangs off Appointment1 in TenantA. Every Fact creates its own
/// rows and filters by AppointmentId or a unique token -- the rig shares one SQLite connection and
/// never rolls back, so nothing here asserts a TotalCount or a list position.
///
/// NO AUTHORIZATION FACTS -- AddAlwaysAllowAuthorization() makes every [Authorize] a no-op.
/// </summary>
public abstract class AppointmentClaimExaminersAppServiceTests<TStartupModule> : CaseEvaluationApplicationTestBase<TStartupModule>
    where TStartupModule : IAbpModule
{
    private readonly IAppointmentClaimExaminersAppService _service;
    private readonly ICurrentTenant _currentTenant;
    private readonly IDataFilter _dataFilter;

    protected AppointmentClaimExaminersAppServiceTests()
    {
        _service = GetRequiredService<IAppointmentClaimExaminersAppService>();
    ...
    private static string Token(string label) => $"ace-{label}-{Guid.NewGuid():N}"[..40];

    // The repo's [PhoneNumber] requires EXACTLY ten digits once punctuation is stripped, so a
    // seven-digit placeholder fails validation rather than the code under test.
    private const string TenDigitPhone = "2135550134";

    private async Task<AppointmentClaimExaminerDto> CreateAsync(string name)
    {
        return await _service.CreateAsync(new AppointmentClaimExaminerCreateDto
        {
    ...
    public async Task CreateAsync_PersistsAgainstTheAppointment()
    {
        using (_currentTenant.Change(TenantsTestData.TenantARef))
        {
            ...
            created.AppointmentId.ShouldBe(
                ...
                "the row must hang off the appointment it was created against, or claim parties "
                + "orphan from their appointment.");
    ...
    public async Task GetListAsync_FilteredByAppointmentId_ReturnsOnlyThatAppointmentsRows()
    {
        using (_currentTenant.Change(TenantsTestData.TenantARef))
        {
            ...
            var result = await _service.GetListAsync(new GetAppointmentClaimExaminersInput
            {
                AppointmentId = AppointmentsTestData.Appointment1Id,
                MaxResultCount = 1000
            });
    ...
    public async Task UpdateAsync_ChangesTheStoredName()
    {
        using (_currentTenant.Change(TenantsTestData.TenantARef))
        {
            ...
            var updated = await _service.UpdateAsync(created.Id, new AppointmentClaimExaminerUpdateDto
            {
    ...
    // ------------------------------------------------------------------------
    // The guard, at the top of both write methods, ahead of any repository call.
    //
    // THE DTO MUST OTHERWISE BE VALID OR THE GUARD IS NEVER REACHED. Name and Email are both
    // [Required] on this DTO, and ABP's method-invocation validator runs BEFORE the method body.
    // The first version of these two Facts omitted Email, so ABP threw AbpValidationException and
    // the Guid.Empty guard never executed -- the Fact was asserting the validator while appearing
    // to assert the service. That is the "guard tested behind a gate that runs first" shape, and
    // it is why both DTOs below are complete apart from the one field under test.
    //
    // The exception TYPE is asserted rather than bare Exception -- an untyped assertion would be
    // satisfied by exactly the validator failure this comment describes.
    // ------------------------------------------------------------------------
    ...
    public async Task CreateAsync_WithEmptyAppointmentId_ThrowsUserFriendlyException()
    {
        using (_currentTenant.Change(TenantsTestData.TenantARef))
        {
            await Should.ThrowAsync<UserFriendlyException>(
                () => _service.CreateAsync(new AppointmentClaimExaminerCreateDto
                {
    ...
    public async Task UpdateAsync_WithEmptyAppointmentId_ThrowsUserFriendlyException()
    {
        using (_currentTenant.Change(TenantsTestData.TenantARef))
        {
            ...
            await Should.ThrowAsync<UserFriendlyException>(
                () => _service.UpdateAsync(created.Id, new AppointmentClaimExaminerUpdateDto
                {
                    AppointmentId = Guid.Empty,
