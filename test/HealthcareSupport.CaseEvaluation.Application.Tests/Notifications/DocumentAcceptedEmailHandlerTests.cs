...
/// <summary>
/// Unit coverage for <see cref="DocumentAcceptedEmailHandler"/>: the single To+CC email sent when
/// office staff accept an uploaded document.
///
/// <para><b>WHAT IS PINNED.</b> Who the email goes to (To = the uploader, CC = every other party
/// EXCEPT the office mailbox, because the office performed the accept), which of the three
...
/// </summary>
public class DocumentAcceptedEmailHandlerTests
{
    ...
    private sealed class Rig
    {
        ...
        public DocumentAcceptedEmailHandler Build() => new(
            Dispatcher,
            ContextResolver,
            RecipientResolver,
            MissingResolver,
            CurrentTenant,
            NullLogger<DocumentAcceptedEmailHandler>.Instance,
            UrlBuilder);
    }
    ...
    private static SentEmail SingleSend(INotificationDispatcher dispatcher)
    {
        var calls = dispatcher.ReceivedCalls()
            ...
            .ToList();
        calls.Count.ShouldBe(1, "exactly one To+CC email is expected for one accepted document");
    ...
    public async Task HandleEventAsync_PackageDocumentWithNothingMissing_EmailsTheUploader_PositiveControl()
    {
        ...
        sent.TemplateCode.ShouldBe(NotificationTemplateConsts.Codes.PatientDocumentAccepted);
    ...
    /// <summary>
    /// The office performed the accept, so it is left OFF the CC (contrast the upload email, which
    ...
    public async Task HandleEventAsync_CcDropsTheOfficeMailboxAndKeepsEveryOtherParty()
    {
        ...
        await rig.RecipientResolver.Received(1).ResolveAsync(evt.AppointmentId, NotificationKind.DocumentAccepted);
    }

    [Theory]
    [InlineData(false, false, NotificationTemplateConsts.Codes.PatientDocumentAccepted)]
    [InlineData(true, false, NotificationTemplateConsts.Codes.PatientNewDocumentAccepted)]
    [InlineData(false, true, NotificationTemplateConsts.Codes.JointAgreementLetterAccepted)]
    ...
    /// <summary>
    /// A package document accepted while required documents are still outstanding switches to the
    ...
    public async Task HandleEventAsync_PackageDocumentWithDocumentsStillMissing_UsesTheRemainingDocsTemplate()
    {
        ...
        var sent = SingleSend(rig.Dispatcher);
        sent.TemplateCode.ShouldBe(NotificationTemplateConsts.Codes.PatientDocumentAcceptedRemainingDocs);
    ...
    public async Task HandleEventAsync_CarriesTheUploaderAndDocumentLabelAndTagsTheSend()
    {
        ...
        sent.ContextTag.ShouldBe(
            $"DocumentAccepted/{NotificationTemplateConsts.Codes.PatientDocumentAccepted}/{evt.AppointmentDocumentId}");
    }
}
