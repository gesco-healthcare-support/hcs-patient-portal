...
} from '../../../proxy/wcab-offices/models';
import { WcabOfficeService } from '../../../proxy/wcab-offices/wcab-office.service';

export abstract class AbstractWcabOfficeViewService {
  protected readonly proxyService = inject(WcabOfficeService);
  ...
  data: PagedResultDto<WcabOfficeWithNavigationPropertiesDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<WcabOfficeWithNavigationPropertiesDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetWcabOfficesInput;
  ...
  protected bulkDeleteRequest() {
    const ids = this.selected().map(({ wcabOffice: { id } }) => id);
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<WcabOfficeWithNavigationPropertiesDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetWcabOfficesInput;
    this.list.get();
  }
