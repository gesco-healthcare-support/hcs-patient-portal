import type { GetWcabOfficesInput, WcabOfficeCreateDto, WcabOfficeDto, WcabOfficeExcelDownloadDto, WcabOfficeUpdateDto, WcabOfficeWithNavigationPropertiesDto } from './models';
...
export class WcabOfficeService {
  ...
  deleteAll = (input: GetWcabOfficesInput, config?: Partial<Rest.Config>) =>
    this.restService.request<any, void>({
      ...
      params: { filterText: input.filterText, name: input.name, abbreviation: input.abbreviation, address: input.address, city: input.city, zipCode: input.zipCode, isActive: input.isActive, stateId: input.stateId, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
  ...
  getListAsExcelFile = (input: WcabOfficeExcelDownloadDto, config?: Partial<Rest.Config>) =>
    this.restService.request<any, Blob>({
      ...
      params: { downloadToken: input.downloadToken, filterText: input.filterText, name: input.name, abbreviation: input.abbreviation, address: input.address, city: input.city, zipCode: input.zipCode, isActive: input.isActive, stateId: input.stateId },
    },
    { apiName: this.apiName,...config });
