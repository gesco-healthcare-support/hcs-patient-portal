...
export abstract class AbstractLocationViewService {
  ...
  data: PagedResultDto<LocationWithNavigationPropertiesDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<LocationWithNavigationPropertiesDto[]>([]);
  ...
  protected bulkDeleteRequest() {
    const ids = this.selected().map(({ location: { id } }) => id);
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<LocationWithNavigationPropertiesDto>) => {
      this.data = list;
