...
4. **Slot booking is capacity-aware (2026-05-15 slot rework plan 3).** `CreateAsync` does NOT mutate the slot's `BookingStatusId`. The slot stays `Available` until a doctor's-admin manually closes it (`BookingStatusId = Reserved`). Whether a slot is bookable is determined by `ValidateDoctorAvailabilityForBookingAsync`'s capacity-aware predicate: `Reserved` blocks with `AppointmentBookingSlotClosed`; capacity exhausted (`active count >= Capacity`) blocks with `AppointmentBookingSlotFull`; a non-empty `AppointmentTypes` set that does not contain the requested type blocks with `AppointmentBookingSlotTypeMismatch`. `Booked` is a legacy enum value the new model treats like `Available` (active-count is authoritative). The five slot-freed terminal statuses (`Rejected`, `CancelledNoBill`, `CancelledLate`, `RescheduledNoBill`, `RescheduledLate`) do not count toward `activeAppointmentCount`, so cancellation / reschedule naturally frees capacity without a status flip. The previous `SlotCascadeHandler` 14-state mapping is gone (now a log-only stub).
