...
/// <summary>
/// Entity configuration shared by <see cref="CaseEvaluationDbContext"/> (the host database)
/// and <see cref="CaseEvaluationTenantDbContext"/> (the per-office databases).
///
/// <para>ABP's database-per-tenant layout needs the same model declared for both contexts, and
/// before this file existed it was declared twice -- the two contexts were 80% and 85%
/// duplicates of each other. That is not merely repetition: it is how the host and office
/// schemas silently drift apart, which has bitten this project before. One definition, two
/// callers.</para>
///
/// <para>What deliberately does NOT live here: the <c>HasOne&lt;Tenant&gt;()</c> foreign keys on
/// Doctor and Patient, because the Saas <c>Tenant</c> table exists only in the host database;
/// Doctor's child collections, which only the tenant context declares; and the two host-only
/// entities. Each context adds its own after calling in here.</para>
/// </summary>
internal static class CaseEvaluationSharedModelConfiguration
{
    ...
    /// </summary>
    internal static void ConfigureCaseEvaluationShared(this ModelBuilder builder)
    {
        ...
        // Mirrors the tenant-context block verbatim per the dual-DbContext
        // convention so the shared AppNotificationOutboxItems table is
        // column-identical across host + office DBs.
        ...
        // Case Tracker outbound ledger. Configured identically here and in
        // CaseEvaluationTenantDbContext so the shared AppIntegrationOutboxItems table is
        // column-identical across host + office DBs.
        ...
        builder.Entity<Appointment>(b =>
        {
            ...
            // IP6 (2026-06-05): optional -- booking no longer mints a patient login.
        ...
        builder.Entity<HealthcareSupport.CaseEvaluation.AppointmentDocuments.AppointmentDocument>(b =>
        {
            ...
            // G-03-03/05 (PR2): document-type linkage. AppointmentDocumentTypeId
            // is a loose Guid reference (no FK -- the type list is tenant-scoped
            // and retired independently); OtherDocumentTypeName holds the "Other"
            // free-text label; SourceDocumentId is internal bookkeeping for
            // queued package rows.
        ...
        // ChangeRequestConsentRound (phase 4c, 2026-08-05) -- one row per staff-confirmed
        // reschedule date plus the consent both sides gave for it. Rounds are rows rather than
        // columns so "which dates were proposed and who declined them" stays queryable.
        builder.Entity<ChangeRequestConsentRound>(b =>
        {
            ...
            b.Property(x => x.ProposedByUserId).HasColumnName(nameof(ChangeRequestConsentRound.ProposedByUserId));
            b.Property(x => x.ProposedReason).HasColumnName(nameof(ChangeRequestConsentRound.ProposedReason)).HasMaxLength(AppointmentChangeRequestConsts.ReasonMaxLength);
            b.Property(x => x.SendAttempts).HasColumnName(nameof(ChangeRequestConsentRound.SendAttempts));
            b.Property(x => x.SupersededAt).HasColumnName(nameof(ChangeRequestConsentRound.SupersededAt));
            b.Property(x => x.SideAConsentStatus).HasColumnName(nameof(ChangeRequestConsentRound.SideAConsentStatus));
            b.Property(x => x.SideAConsentTokenHash).HasColumnName(nameof(ChangeRequestConsentRound.SideAConsentTokenHash)).HasMaxLength(AppointmentChangeRequestConsts.ConsentTokenHashLength);
            b.Property(x => x.SideAConsentExpiresAt).HasColumnName(nameof(ChangeRequestConsentRound.SideAConsentExpiresAt));
            b.Property(x => x.SideAConsentRespondedAt).HasColumnName(nameof(ChangeRequestConsentRound.SideAConsentRespondedAt));
            b.Property(x => x.SideAConsentRespondedByEmail).HasColumnName(nameof(ChangeRequestConsentRound.SideAConsentRespondedByEmail)).HasMaxLength(AppointmentChangeRequestConsts.ConsentRespondedByEmailMaxLength);
            b.Property(x => x.SideBConsentStatus).HasColumnName(nameof(ChangeRequestConsentRound.SideBConsentStatus));
            b.Property(x => x.SideBConsentTokenHash).HasColumnName(nameof(ChangeRequestConsentRound.SideBConsentTokenHash)).HasMaxLength(AppointmentChangeRequestConsts.ConsentTokenHashLength);
            b.Property(x => x.SideBConsentExpiresAt).HasColumnName(nameof(ChangeRequestConsentRound.SideBConsentExpiresAt));
            b.Property(x => x.SideBConsentRespondedAt).HasColumnName(nameof(ChangeRequestConsentRound.SideBConsentRespondedAt));
            b.Property(x => x.SideBConsentRespondedByEmail).HasColumnName(nameof(ChangeRequestConsentRound.SideBConsentRespondedByEmail)).HasMaxLength(AppointmentChangeRequestConsts.ConsentRespondedByEmailMaxLength);
            // Token resolution seeks these two columns on every public consent click.
            b.HasIndex(x => x.SideAConsentTokenHash);
            b.HasIndex(x => x.SideBConsentTokenHash);
            b.HasIndex(x => x.AppointmentChangeRequestId);
            b.HasIndex(x => new { x.TenantId, x.AppointmentChangeRequestId, x.RoundNumber }).IsUnique().HasFilter("[IsDeleted] = 0");
    ...
    /// <summary>
    /// Doctor itself. The host gates this behind IsHostDatabase(); the office context configures
    /// it unconditionally, so the call site -- not this method -- decides.
    ///
    /// <para>Split from <see cref="ConfigureDoctorJoinEntities"/> on purpose. The office context
    /// adds Doctor's child collections, and those must be declared BEFORE the join entities:
    /// <c>HasMany(...).WithOne()</c> names no inverse navigation, so if the join entities have
    /// already declared the relationship in full, EF reads it as a second relationship and
    /// invents a <c>DoctorId1</c> shadow column. Call order here is load-bearing.</para>
    /// </summary>
    internal static void ConfigureDoctor(this ModelBuilder builder)
    {
        builder.Entity<Doctor>(b =>
        {
            ...
            // Declared once here, so the host and office schemas cannot drift
    ...
    /// <summary>
    /// Doctor's two join entities. Must be configured AFTER <see cref="ConfigureDoctor"/> and
    /// after any context-specific <c>HasMany</c> on Doctor -- see the remarks there.
    /// </summary>
    internal static void ConfigureDoctorJoinEntities(this ModelBuilder builder)
    {
    ...
    /// <summary>
    /// Patient. Gated behind IsHostDatabase() by the host context, unconditional in the office context.
    /// </summary>
    internal static void ConfigurePatient(this ModelBuilder builder)
    {
