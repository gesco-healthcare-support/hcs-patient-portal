...
helm upgrade --install ${FinalReleaseName} ${ChartName} --namespace ${Namespace} --create-namespace --set global.dotnetEnvironment=${DotnetEnvironment} -f "caseevaluation/values.localdev.yaml" -f "$ChartName/values.${ReleaseName}.yaml"
exit $LASTEXITCODE