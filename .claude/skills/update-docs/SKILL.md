...
- **`modified`**:
  Use `git diff --name-only HEAD~1` to find changed source files since last commit.
...
2. If verify reports **CRITICAL** issues:
   ...
   - If "fix": the verify skill's auto-fix will handle navigation issues;
     for accuracy issues, re-run `/generate-feature-doc` on the affected feature
...
- If available: Run `/sync-to-vault`
