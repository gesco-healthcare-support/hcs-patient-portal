...
business feature folder has a row in the table. Flag missing rows.

EXCLUDE (ABP-internal, not user features): Identity, OpenIddict, Saas, Settings,
Tenants, BackgroundJobs, AuditLogs, FeatureManagement.

Before flagging a folder as a missing row, verify it is not in the EXCLUDE list
above and that it actually exists on disk via Glob. If a folder is ambiguous
(e.g. ABP added a new internal module not on this list), record it as a
manual-review item rather than auto-flagging it as a missing feature.
...
### 2c. Spot-check up to 3 claims against source code

Pick the 3 highest-priority claims available from the source CLAUDE.md, walking
the priority list below in order. If a higher-priority claim is unavailable in
this CLAUDE.md, fall back to the next priority. If fewer than 3 claims across
all four priorities are available, spot-check fewer and record the actual count
in the report (e.g. `spot-checks: 2/3 -- only 2 verifiable claims found`).
...
**Ownership boundary (prevents skill conflicts on the same file):** verify-docs
only FLAGS missing bidirectional FKs in the audit report. It does NOT auto-add
`## Related Features` links during Step 6. Only `sync-feature-to-docs` (Step 4.5
in that skill) adds the cross-feature link blocks. If verify-docs and
sync-feature-to-docs both wrote to `## Related Features`, they would compete and
may produce duplicate or conflicting links.
...
### 3b. Verify claims (deterministic sample for large files)

- If the file has **10 or fewer** testable claims: verify all of them.
- If the file has **more than 10**: verify the first 10 plus a deterministic
  stride sample of up to 5 more (15 total max per file). Compute stride as
  `N = max(1, floor(total_claims / 5))` and select claims at indexes
  `10, 10+N, 10+2N, 10+3N, 10+4N` (clamped to the claim list, deduped).
  Determinism matters because audits must be reproducible across runs;
  random sampling makes regression comparisons impossible.
...
1. **Transitive reachability pre-check.** Using the link map built in Step 1b,
   check whether ANY other `docs/` file links to this file (inbound link count
   > 0). If at least one inbound link exists from somewhere in the docs tree,
   the file is reachable via transitive navigation -- do NOT append a back-link.
   This avoids redundant tail-of-file links on, for example,
   `docs/features/{x}/api.md` whose sibling `overview.md` already links to
   INDEX.md.
2. **Add only when fully orphaned.** If the file has zero inbound links from
   any other docs file, add at the bottom, before any
   `<!-- DOCS:MANUAL:START -->` block:
   ```
   ---
...
### 6c.1. Do not stack UNVERIFIED markers

Before writing a new `<!-- UNVERIFIED: ... -->` comment on a line:
- Search the line for an existing `<!-- UNVERIFIED: ... -->` marker.
- If one exists, REPLACE it in place with the current run's reason (do not
  append a second comment alongside it).
- If none exists, append the new marker as described in 6c.
This prevents marker accumulation across repeated audit runs.

### 6d. HIPAA / synthetic-data guard before writing any corrected value

Before Step 6a, 6b, or 6c writes any value into a docs file:
- Verify the value is **structural** (class name, base class, port, file path,
  enum value, config key, count, command). Structural values are safe.
- If the value is **data-shaped** (resembles a person's name, SSN, DOB, phone,
  address, email, MRN, claim number, or any patient identifier), STOP. Write
  the literal placeholder `<example>` instead and add the comment
  `<!-- redacted: data-shaped value, replaced with placeholder -->`.
- If unsure whether a value is structural vs data-shaped, treat it as
  data-shaped and use `<example>`. False positives here cost nothing; a real
  PHI leak into committed docs is a HIPAA incident.

This guard is a safety net. Source CLAUDE.md should already be PHI-clean (the
generate-feature-doc skill has its own Step 0 safety check), but verify-docs
must not assume upstream cleanliness.

### 6e. Protected content
